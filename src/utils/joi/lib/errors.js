...
export const Err = class {
  ...
  toString() {
    ...
    if (format === undefined) {
      return `Mã Lỗi "${this.type}" không được định nghĩa, mã lỗi bạn tuỳ chỉnh bị thiếu trong định nghĩa ngôn ngữ`;
    }
...
export const create = function (type, context, state, options, flags, message, template) {

  return new Err(type, context, state, options, flags, message, template);
};


export const process = function (errors, object) {
