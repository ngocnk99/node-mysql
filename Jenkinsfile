                    ...
                    sshagent(['dev-10-0-200-114']) {
                        script {
                          if (env.BRANCH_NAME == "test" ) {
                            ...
                            "cd /home/env/apichoso; bash -s" < ./scripts/deploy-dev.sh $BUILD_NUMBER $BRANCH_NAME'
                          } else {
                            sh 'ssh -o StrictHostKeyChecking=no -l dev 10.0.200.114 \
                            "cd /home/env/apichoso; bash -s" < ./scripts/deploy-dev.sh $BUILD_NUMBER'
                          }
                        }
                    }
                }
            ...
            steps {
                script {
                    sshagent(['prod-10-0-200-112']) {
                        ...
                        "cd /home/env/apichoso; bash -s" < ./scripts/deploy-prod.sh $BUILD_NUMBER'
                    }
                }
            }
        }
