...
DOCKER_IMAGE=10.0.200.107:5000/choso-api-"$BUILD_TAG":"$BUILD_NUMBER"
ENV_FILE=/home/env/apichoso/.env.production # Default to dev environment
CONTAINER_NAME=choso-api-"$BUILD_TAG"

if [ "$BUILD_TAG" = "master" ]; then
  ENV_FILE=/home/env/apichoso/.env.production # Production environment file
elif [ "$BUILD_TAG" = "test" ]; then
  ENV_FILE=/home/env/apichoso/.env.production # Staging environment file
fi
